...
import uz.gita.online_shopping_admin.databinding.ScreenMapBinding
import uz.gita.online_shopping_admin.presenter.MapViewModelImpl
import uz.gita.online_shopping_admin.utils.extensions.hasPermission
import uz.gita.online_shopping_admin.utils.extensions.isLocationEnabled
import uz.gita.online_shopping_admin.utils.extensions.toAddress
...
class MapScreen : Fragment(R.layout.screen_map) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        val map = childFragmentManager.findFragmentById(R.id.map) as MapHelper
        map.getMapAsync(map)
        map.onMapReady {
            googleMap = it
        ...
        viewBinding.btnConfirm.setOnClickListener {
            findNavController().previousBackStackEntry?.savedStateHandle?.set(
                "map",
                googleMap.cameraPosition.target.toAddress()
            )
            findNavController().previousBackStackEntry?.savedStateHandle?.set(
                "way",
                viewBinding.tvAddressName.text.toString()
            )
